...
interface ActiveParcelDao {
    ...
    @Query("SELECT * FROM active_parcels")
    suspend fun getAllParcels(): List<ActiveParcelEntity>
    ...
    @Query("SELECT geomWKT FROM active_parcels WHERE geomWKT IS NOT NULL AND geomWKT != ''")
    suspend fun getAllActiveParcelGeometries(): List<String>
    ...
    @Query("DELETE FROM active_parcels")
