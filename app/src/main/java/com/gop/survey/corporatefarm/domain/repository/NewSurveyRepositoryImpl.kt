...
class NewSurveyRepositoryImpl @Inject constructor(
    ...
    override suspend fun deleteSurvey(survey: NewSurveyNewEntity): Resource<Unit> {
        return try {
            ...
            Log.d(
                TAG,
                "Survey: parcelId=${survey.parcelId}, parcelNo=${survey.parcelNo}, operation=${survey.parcelOperation}"
            )
            ...
            when (survey.parcelOperation) {
                "Merge" -> {
                    ...
                    if (survey.parcelOperationValue.isNotBlank()) {
                        ...
                        Log.d(
                            TAG,
                            "Found ${mergedParcelIds.size} merged child parcels: $mergedParcelIds"
                        )
                        ...
                        for (childParcelId in mergedParcelIds) {
                            ...
                            if (childSurveys.isEmpty()) {
                                ...
                                Log.d(
                                    TAG,
                                    "Child parcel $childParcelId has other surveys, skipping reset"
                                )
                            }
                        }
                    }
...
//    override suspend fun getCorporateParcels(): List<CorporateParcel> {
//        return try {
//            val token = sharedPreferences.getString(Constants.SHARED_PREF_TOKEN, "") ?: ""
//            Log.d("Repository", "Token: $token")
//            Log.d("Repository", "Calling getCorporateParcels API...")
//
//            val response = api.getCorporateParcels("Bearer $token")
//
//            Log.d("Repository", "Response code: ${response.code()}")
//            Log.d("Repository", "Response successful: ${response.isSuccessful}")
//            Log.d("Repository", "Response body null? ${response.body() == null}")
//            Log.d("Repository", "Data size: ${response.body()?.data?.size}")
//
//            if (response.isSuccessful) {
//                val parcels = response.body()?.data ?: emptyList()
//                Log.d("Repository", "✅ Returning ${parcels.size} parcels")
//                parcels
//            } else {
//                val error = response.errorBody()?.string()
//                Log.e("Repository", "❌ Error response: $error")
//                emptyList()
//            }
//        } catch (e: Exception) {
...
//    }
    ...
    override suspend fun uploadSurvey(
        ...
        return try {
            ...
            Log.d(
                TAG,
                "Survey object: parcelId=${survey.parcelId}, parcelOperation=${survey.parcelOperation}"
            )
            ...
            var isSplitAfterMultiMerge = false
            ...
            val subparcels = withContext(Dispatchers.IO) {
                when (survey.parcelOperation) {
                    ...
                    "Merge" -> processMultiSelectedParcels(survey)

                    "MultiMerge" -> processMultiMergeParcels(survey)

                    "Same" -> {
                        val localParcel = activeParcelDao.getParcelById(survey.parcelId)
                        val isSplitChild = localParcel != null &&
                                localParcel.subParcelNo.isNotBlank() &&
                                localParcel.subParcelNo != "0"
                        val parentWasMultiMerge = !localParcel?.multiMergeParcelNos.isNullOrBlank()

                        when {
                            // ✅ CASE 1: Split child whose parent was a MultiMerge survivor
                            isSplitChild && parentWasMultiMerge -> {
                                Log.d(TAG, "⚠️ Detected: Split parcel from a MultiMerge parent")
                                Log.d(TAG, "   MultiMergeParcelNos: ${localParcel?.multiMergeParcelNos}")
                                Log.d(TAG, "   Will use processSplitParcels() with SplitAfterMultiMerge marker")

                                isSplitAfterMultiMerge = true
                                originalParcelIdToDeactivate = null
                                processSplitParcels(survey)
                            }

                            // ✅ CASE 2: Regular split child (parent was a normal parcel)
                            isSplitChild -> {
                                ...
                                // Find the original parent parcel
                                val allParcelsWithSameNumber =
                                    activeParcelDao.getActiveParcelsByMauzaAndArea(
                                        localParcel!!.mauzaId,
                                ...
                                if (originalParent != null) {
                                    ...
                                    if (inactiveParent != null) {
                                        Log.d(
                                            TAG,
                                            "   Found inactive parent: ID=${inactiveParent.id}, isActivate=${inactiveParent.isActivate}"
                                        )
                                    }
                                }
                            ...
                            // ✅ CASE 3: Normal "Same" — not a split child at all
                            else -> {
            ...
            val posts = withContext(Dispatchers.IO) {
                buildCorporateSurveyPosts(context, survey, subparcels)
            }
            ...
            posts.forEachIndexed { index, post ->
                Log.d(
                    TAG,
                    "Upload ${index + 1}: ParcelNo=${post.parcelNo}, SubParcel=${post.subParcelNo}, Operation=${post.parcelOperation}, OpValue=${post.parcelOperationValue}, HasGeom=${!post.geomWKT.isNullOrBlank()}"
                )
            }
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                withContext(Dispatchers.IO) {
                    // Mark all subparcels as uploaded (jin ki pkId > 0 hai)
                    subparcels.forEach { subSurvey ->
                        if (subSurvey.pkId > 0) {
                            dao.markAsUploaded(subSurvey.pkId)
                            lessorDao.markSurveyLessorsSynced(subSurvey.pkId)
                            Log.d(TAG, "Marked subparcel pkId=${subSurvey.pkId} as uploaded")
                        }
                    }

                    // ✅ NEW: Also mark the ORIGINAL survey as uploaded
                    if (survey.pkId > 0) {
                        dao.markAsUploaded(survey.pkId)
                        lessorDao.markSurveyLessorsSynced(survey.pkId)
                        Log.d(TAG, "✅ Marked ORIGINAL survey pkId=${survey.pkId} as uploaded")
                    }

                    if ((survey.parcelOperation == "Split" ||
                                isSplitParcelWithSameOperation ||
                                isSplitAfterMultiMerge) &&
                        originalParcelIdToDeactivate != null
                    ) {
                        deactivateOriginalParcelWithVerification(originalParcelIdToDeactivate!!)
                    }

                    if (isSplitAfterMultiMerge) {
                        val localParcel = activeParcelDao.getParcelById(survey.parcelId)
                        val multiMergeParcelNos = localParcel?.multiMergeParcelNos
                        if (!multiMergeParcelNos.isNullOrBlank() && localParcel != null) {
                            deactivateMultiMergeOriginalsLocally(
                                multiMergeParcelNos,
                                localParcel.mauzaId,
                                localParcel.areaAssigned
                            )
                ...
                withContext(Dispatchers.Main) {
                    try {
                        val refreshIntent = android.content.Intent("REFRESH_MAP")
                        androidx.localbroadcastmanager.content.LocalBroadcastManager
                            .getInstance(context)
                            .sendBroadcast(refreshIntent)
                        Log.d(TAG, "📡 Sent REFRESH_MAP broadcast")
                    } catch (e: Exception) {
                ...
                Log.e(TAG, "401 body: $errorBodyStr")
                handleUnauthorizedResponse(errorBodyStr)
            ...
            Log.e(TAG, "HttpException - Code: ${e.code()}, Body: $errorBody", e)

            val msg = if (errorBody.contains("isUpdateRequired", ignoreCase = true))
            ...
            Log.e(TAG, "Unexpected exception during survey upload", e)
            Resource.Error(e.localizedMessage ?: "Unexpected error occurred while uploading")
        }
    }

    private suspend fun deactivateMultiMergeOriginalsLocally(
        multiMergeParcelNos: String,
        mauzaId: Long,
        areaName: String
    ) {
        withContext(Dispatchers.IO) {
            try {
                val parcelNosStr = multiMergeParcelNos
                    .split(",")
                    .map { it.trim() }
                    .filter { it.isNotEmpty() }

                Log.d(TAG, "🔧 Deactivating original MultiMerge parents locally: $parcelNosStr")

                // ✅ Get ALL parcels (active + inactive both)
                val allMatchingParcels = activeParcelDao.getAllParcels()
                    .filter {
                        parcelNosStr.contains(it.parcelNo.toString()) &&
                                (it.subParcelNo.isBlank() || it.subParcelNo == "0")
                    }

                Log.d(TAG, "Found ${allMatchingParcels.size} matching parcels (active or not)")

                allMatchingParcels.forEach { parent ->
                    Log.d(TAG, "   Processing: ID=${parent.id}, ParcelNo=${parent.parcelNo}, SubParcel='${parent.subParcelNo}', isActive=${parent.isActivate}")
                    if (parent.isActivate == true) {
                        activeParcelDao.deactivateParcel(parent.id)
                        Log.d(TAG, "   ✅ Deactivated parcel ID=${parent.id}")
                    } else {
                        Log.d(TAG, "   ℹ️ Already inactive, skipping")
                    }
                }

                Log.d(TAG, "✅ Deactivation processing complete")
            } catch (e: Exception) {
            ...
            try {
                ...
                if (parcelAfter != null) {
                    ...
                    if (parcelAfter.isActivate == false) {
                        ...
                        Log.e(
                            TAG,
                            "❌❌ ERROR: Original parcel still active after deactivation attempt!"
                        )
                    }
                } else {
                ...
                Log.d(
                    TAG,
                    "Active split parcels for ParcelNo ${parcelBefore.parcelNo}: ${activeSplitParcels.size}"
                )
    ...
    private suspend fun processSplitParcels(survey: NewSurveyNewEntity): List<NewSurveyNewEntity> {
        ...
        val wasMultiMerge = !originalParcel.multiMergeParcelNos.isNullOrBlank()
        val multiMergeContext = originalParcel.multiMergeParcelNos ?: ""

        Log.d(TAG, "Was MultiMerge before split: $wasMultiMerge")
        if (wasMultiMerge) {
            Log.d(TAG, "MultiMerge parcel_nos to deactivate on server: $multiMergeContext")
        }
        ...
        splitParcels.forEachIndexed { index, parcel ->
            ...
            Log.d(
                TAG,
                "   GeomWKT: ${if (parcel.geomWKT.isNullOrEmpty()) "❌ NULL/EMPTY" else "✅ Present (${parcel.geomWKT.length} chars)"}"
            )
        }
        Log.d(TAG, "Total split parcels found: ${splitParcels.size}")
        ...
        splitParcels.forEach { splitParcel ->
            // ✅ Determine if this split child is the one user surveyed
            val isThisOneSurveyed = (splitParcel.id == survey.parcelId)

            Log.d(TAG, "Processing splitParcel ID=${splitParcel.id}, SubParcel=${splitParcel.subParcelNo}, isSurveyedByUser=$isThisOneSurveyed")

            val splitSurvey = if (isThisOneSurveyed) {
                // ✅ SURVEYED: keep all the survey data
                survey.copy(
                    pkId = survey.pkId,
                    parcelId = splitParcel.id,
                    parcelNo = splitParcel.parcelNo.toString(),
                    subParcelNo = splitParcel.subParcelNo,
                    parcelOperation = if (wasMultiMerge) "SplitAfterMultiMerge" else "New",
                    parcelOperationValue = if (wasMultiMerge) multiMergeContext else survey.parcelId.toString()
                )
            } else {
                // ✅ UNSURVEYED: clear all survey-specific fields so server treats it as not surveyed
                survey.copy(
                    pkId = 0,
                    parcelId = splitParcel.id,
                    parcelNo = splitParcel.parcelNo.toString(),
                    ...
                    parcelOperation = if (wasMultiMerge) "SplitAfterMultiMerge" else "New",
                    parcelOperationValue = if (wasMultiMerge) multiMergeContext else survey.parcelId.toString(),
                    // ✅ Clear survey data so server marks IsSurveyed=false
                    propertyType = "",
                    ownershipStatus = "",
                    variety = "",
                    cropType = "",
            ...
            Log.d(TAG, "✅ Created record: parcelId=${splitParcel.id}, isSurveyed=$isThisOneSurveyed")
        }
        return splitSurveys
    }

    private suspend fun processMultiSelectedParcels(survey: NewSurveyNewEntity): List<NewSurveyNewEntity> {
        ...
        Log.d(
            TAG,
            "Main parcel => ParcelNo=${mainParcel.parcelNo}, SubParcelNo='${mainParcel.subParcelNo}', isActivate=${mainParcel.isActivate}"
        )
        ...
        if (mainIsSplitParcel) {
            ...
            Log.d(
                TAG,
                "Found ${allSiblingsSplitParcels.size} unsurveyed sibling split parcel(s): ${allSiblingsSplitParcels.map { "ID=${it.id}, SubParcel=${it.subParcelNo}" }}"
            )

            // Determine opValue for original parent deactivation
            val originalIsStillActive = originalParent?.isActivate == true
            val originalParentIdStr =
                if (originalIsStillActive) originalParent!!.id.toString() else ""

            Log.d(
                TAG,
                "Original parent deactivation: ${if (originalIsStillActive) "YES → ID=${originalParent!!.id}" else "NO (already inactive or not found)"}"
            )
            ...
            allSiblingsSplitParcels.forEachIndexed { index, sibling ->
                ...
                Log.d(
                    TAG,
                    "   parcelId=${sibling.id}, ParcelNo=${sibling.parcelNo}, SubParcelNo=${sibling.subParcelNo}"
                )
            ...
            Log.d(
                TAG,
                "   parcelId=${mainParcel.id}, ParcelNo=${mainParcel.parcelNo}, SubParcelNo=${mainParcel.subParcelNo}"
            )
            ...
            mergedParcelIds.forEach { mergedParcelId ->
                addMultiSelectedParcel(survey, mergedParcelId, mergeSurveys)
            }
            ...
            Log.d(
                TAG,
                "✅ Added main normal parcel as 'Merge' with value: ${survey.parcelOperationValue}"
            )
        ...
        mergeSurveys.forEachIndexed { i, s ->
            Log.d(
                TAG,
                "  Record ${i + 1}: parcelId=${s.parcelId}, ParcelNo=${s.parcelNo}, SubParcel=${s.subParcelNo}, op=${s.parcelOperation}, opValue='${s.parcelOperationValue}', propertyType='${s.propertyType}'"
            )
        }
        return mergeSurveys
    }
    ...
    private suspend fun addMultiSelectedParcel(
        survey: NewSurveyNewEntity,
        mergedParcelId: Long,
        ...
        Log.d(
            TAG,
            "Processing merged parcel ID=$mergedParcelId: ParcelNo=${mergedParcel.parcelNo}, SubParcelNo='${mergedParcel.subParcelNo}', isSplit=$mergedIsSplit"
        )
        ...
        if (mergedIsSplit) {
            ...
            val originalParent = allParcelsWithSameNumber.firstOrNull()
            val opValue =
                if (originalParent?.isActivate == true) originalParent.id.toString() else ""
            ...
            Log.d(
                TAG,
                "✅ Added split merged parcel as 'New': ID=${mergedParcel.id}, opValue='$opValue'"
            )
        } else {
            ...
            Log.d(
                TAG,
                "✅ Added normal merged parcel as 'Merge': ID=$mergedParcelId, ParcelNo=${mergedParcel.parcelNo}"
            )
        }
    }
    ...
    private suspend fun getParcelDataForOperation(subSurvey: NewSurveyNewEntity): ParcelData {
        ...
        Log.d(
            TAG,
            "ParcelId: ${subSurvey.parcelId}, Op: ${subSurvey.parcelOperation}, SubParcel: ${subSurvey.subParcelNo}"
        )

        return when (subSurvey.parcelOperation) {

            "New", "SplitAfterMultiMerge" -> {
                ...
                if (parcel != null) {
                    ...
                    Log.d(
                        TAG,
                        "✅ [New] Found parcel ID=${parcel.id}, SubParcel=${parcel.subParcelNo}, area=$calculatedArea, geomLen=${parcel.geomWKT?.length ?: 0}"
                    )
            ...
            "Same", "Merge", "MultiMerge" -> {
                val parcel = activeParcelDao.getParcelById(subSurvey.parcelId)
                if (parcel != null) {
                    val calculatedArea = calculateAreaFromGeometry(parcel.geomWKT)
                    Log.d(
                        TAG,
                        "[${subSurvey.parcelOperation}] Found parcel ID=${parcel.id}, SubParcel=${parcel.subParcelNo}, area=$calculatedArea, geomLen=${parcel.geomWKT?.length ?: 0}"
                    )
                    ParcelData(
                        geomWKT = parcel.geomWKT ?: "",
                        centroid = parcel.centroid ?: "",
                    ...
                    Log.e(
                        TAG,
                        "❌ [${subSurvey.parcelOperation}] Parcel NOT FOUND for ID=${subSurvey.parcelId}"
                    )
                    ParcelData("", "", "", "", "")
                }
            }
    ...
    private suspend fun processSameParcels(survey: NewSurveyNewEntity): List<NewSurveyNewEntity> {
        ...
        if (localParcel != null && localParcel.subParcelNo.isNotBlank() && localParcel.subParcelNo != "0") {
            Log.d(
                TAG,
                "Detected split parcel with 'Same' operation - need to upload ALL split parcels"
            )
            Log.d(
                TAG,
                "Parcel details: ID=${localParcel.id}, ParcelNo=${localParcel.parcelNo}, SubParcel=${localParcel.subParcelNo}"
            )
            ...
            Log.d(
                TAG,
                "Found ${allSplitParcels.size} total split parcels for ParcelNo ${localParcel.parcelNo}"
            )
            ...
            allSplitParcels.forEach { splitParcel ->
                ...
                if (existingSurvey != null) {
                    ...
                    Log.d(
                        TAG,
                        "✅ Added SURVEYED split parcel: ID=${splitParcel.id}, SubParcel=${splitParcel.subParcelNo}, HasSurveyData=true"
                    )
                } else {
                    ...
                    Log.d(
                        TAG,
                        "⚠️ Added UNSURVEYED split parcel: ID=${splitParcel.id}, SubParcel=${splitParcel.subParcelNo}, HasSurveyData=false"
                    )
                }
            }
            ...
            Log.d(
                TAG,
                "Converted ${cleanSurveys.size} survey records (surveyed + unsurveyed split parcels)"
            )
            return cleanSurveys
        } else {
            Log.d(TAG, "Normal 'Same' operation - proceeding with existing parcel")
    ...
    private fun processOtherParcels(survey: NewSurveyNewEntity): List<NewSurveyNewEntity> {
        Log.d(TAG, "Processing other operation: ${survey.parcelOperation}")
        return dao.getCompleteRecord(survey.parcelId)
    }

    private suspend fun processMultiMergeParcels(survey: NewSurveyNewEntity): List<NewSurveyNewEntity> {
        Log.d(TAG, "=== PROCESS MULTI-MERGE ===")
        Log.d(TAG, "Survivor parcelId=${survey.parcelId}, ParcelNo=${survey.parcelNo}")
        Log.d(TAG, "All merged parcel_nos: ${survey.parcelOperationValue}")

        val survivorParcel = activeParcelDao.getParcelById(survey.parcelId)
        if (survivorParcel == null) {
            Log.e(TAG, "❌ Survivor parcel not found for ID=${survey.parcelId}")
            return emptyList()
        }

        // Send one survey record — the survivor with merged geometry + list of all parcel_nos
        val multiMergeSurvey = survey.copy(
            parcelId = survivorParcel.id,
            parcelNo = survivorParcel.parcelNo.toString(),
            subParcelNo = "",
            parcelOperation = "MultiMerge",
            parcelOperationValue = survey.parcelOperationValue  // "1,2,3"
        )

        Log.d(TAG, "✅ Created MultiMerge record")
        return listOf(multiMergeSurvey)
    }
    ...
    private suspend fun buildCorporateSurveyPosts(
        ...
        return subparcels.mapIndexed { index, subSurvey ->
            ...
            Log.d(
                TAG,
                "Survey Details: pkId=${subSurvey.pkId}, parcelNo=${subSurvey.parcelNo}, subParcelNo=${subSurvey.subParcelNo}"
            )
            ...
            val images =
                if (isSurveyed) imageDao.getImagesBySurvey(sourceSurveyPkId) else emptyList()
            ...
            // ✅ NEW: Lessors instead of Persons
            val lessorEntities = if (isSurveyed) lessorDao.getLessorsForSurvey(sourceSurveyPkId) else emptyList()
            val lessorDtos = lessorEntities.map { l ->
                LessorPostDto(
                    lessorId = l.lessorId,        // 0 for ad-hoc, real Id for picked
                    lessorName = l.lessorName,
                    lessorCode = l.lessorCode,
                    lessorType = l.lessorType
                )
            }
            Log.d(TAG, "Found ${lessorEntities.size} lessors for survey pkId=$sourceSurveyPkId")


            val persons = emptyList<SurveyPersonPost>()

            val sowingPersonEntities =
                if (isSurveyed) sowingPersonDao.getBySurveyId(sourceSurveyPkId) else emptyList()
            ...
            Log.d(
                TAG,
                "GeomWKT: ${if (parcelData.geomWKT.isNotEmpty()) "Present (${parcelData.geomWKT.length} chars)" else "❌ EMPTY/NULL"}"
            )
            Log.d(
                TAG,
                "Centroid: ${if (parcelData.centroid.isNotEmpty()) parcelData.centroid else "❌ EMPTY/NULL"}"
            )

            val parcelIdToSend =
                if (subSurvey.parcelOperation == "New") 0L else subSurvey.parcelId ?: 0L

            Log.d(TAG, ">>> parcelId being sent to server: $parcelIdToSend")
            Log.d(TAG, ">>> parcelNo being sent to server: ${subSurvey.parcelNo}")
            Log.d(TAG, ">>> operation: ${subSurvey.parcelOperation}")

            val post = CorporateParcelPost(
                parcelId = parcelIdToSend,
                parcelNo = subSurvey.parcelNo,
                subParcelNo = subSurvey.subParcelNo,
                mauzaId = subSurvey.mauzaId,
                parcelOperation = subSurvey.parcelOperation,
                parcelOperationValue = subSurvey.parcelOperationValue,
                geomWKT = if (parcelData.geomWKT.isNotEmpty()) parcelData.geomWKT else null,
                centriod = if (parcelData.centroid.isNotEmpty()) parcelData.centroid else null,
                calculatedArea = parcelData.calculatedArea,
                isGeometryCorrect = subSurvey.isGeometryCorrect,
                distance = 100,
                khewatInfo = parcelData.khewatInfo,
                areaAssigned = localParcel?.areaAssigned ?: "",
                parcelAreaKMF = parcelData.parcelAreaKMF,
                areaName = subSurvey.areaName,
                propertyType = subSurvey.propertyType,
                ownershipStatus = subSurvey.ownershipStatus,
                variety = subSurvey.variety,
                cropType = subSurvey.cropType,
                crop = subSurvey.crop,
                year = subSurvey.year,
                area = subSurvey.area,
                remarks = subSurvey.remarks,
                sowingStatus = subSurvey.sowingStatus ?: "No",
                sowingDate = subSurvey.sowingDate,
                zone = subSurvey.zone ?: localParcel?.zone,
                division = subSurvey.division ?: localParcel?.division,
                section = subSurvey.section ?: localParcel?.section,
                farm = subSurvey.farm ?: localParcel?.farm,
                block = subSurvey.block ?: localParcel?.block,
                plot = subSurvey.plot ?: localParcel?.plot,
                lessorName = localParcel?.lessorName,
                persons = persons,
                pictures = pictures,
                sowingPersons = sowingPersonDtos,
                lessors = lessorDtos,
                irrigationSource = subSurvey.irrigationSource,
            )

            Log.d(TAG, "=== Survey Post ${index + 1} Details ===")
            Log.d(TAG, "ParcelNo: ${post.parcelNo}, SubParcel: ${post.subParcelNo}")
            Log.d(TAG, "ParcelId: ${post.parcelId}, Operation: ${post.parcelOperation}")
            Log.d(TAG, "Pictures Count: ${post.pictures.size}, Persons Count: ${post.persons.size}")
        ...
        return withContext(Dispatchers.Default) {
            try {
                ...
                if (polygon != null && !polygon.isEmpty) {
                    ...
                    val formatted = String.format(Locale.US, "%.4f", areaAcres)
                    Log.d(TAG, "Area calculation: $areaSqFt sq ft = $formatted Acres")
                    formatted
                } else {
                    Log.e(TAG, "Failed to parse geometry")
                    ""
                }
            } catch (e: Exception) {
