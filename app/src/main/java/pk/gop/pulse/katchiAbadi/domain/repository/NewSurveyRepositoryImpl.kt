...
import pk.gop.pulse.katchiAbadi.data.remote.response.CorporateParcel
            ...
            val hasGrowerCodes = withContext(Dispatchers.IO) {
                ...
                if (persons.isEmpty()) {
                    ...
                    if (!isSplitParcel) {
                        ...
                        Log.d(
                            TAG,
                            "ℹ️ Split parcel - may include unsurveyed parcels without persons"
                        )
                ...
                Log.d(
                    TAG,
                    "Found ${persons.size} persons, ${validGrowerCodes.size} have grower codes"
                )
                    ...
                    "Same" -> {
                        ...
                        if (localParcel != null &&
                            ...
                            if (originalParent != null) {
                                ...
                                Log.w(
                                    TAG,
                                    "⚠️ Original parent parcel not found - may already be deactivated"
                                )
...
// ===== UPLOAD TO SERVER =====
            ...
            posts.forEachIndexed { index, post ->
                Log.d(
                    TAG,
                    "Upload ${index + 1}: ParcelNo=${post.parcelNo}, SubParcel=${post.subParcelNo}, Operation=${post.parcelOperation}, HasGeom=${!post.geomWKT.isNullOrBlank()}"
                )
            }
            ...
            Log.e(TAG, "❌ Content-Type: ${response.headers()["Content-Type"]}")
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                // ✅ Now actually uses handleUnauthorizedResponse
                // But we already read errorBody so pass it directly
                Log.e(TAG, "401 body: $errorBodyStr")
                handleUnauthorizedResponse(errorBodyStr)

            } else {
                // ✅ Full error visible on Android
                Log.e(TAG, "❌ UPLOAD FAILED - Code: ${response.code()}")
                Log.e(TAG, "❌ Error Body: $errorBodyStr")
    ...
    private suspend fun processSplitParcels(survey: NewSurveyNewEntity): List<NewSurveyNewEntity> {
        ...
        Log.d(
            TAG,
            "Created ${splitSurveys.size} survey records (including unsurveyed split parcels)"
        )
        return splitSurveys
    }
    ...
    private suspend fun buildCorporateSurveyPosts(
        ...
        return subparcels.mapIndexed { index, subSurvey ->
            ...
            val personsEntities =
                if (isSurveyed) personDao.getPersonsForSurvey(sourceSurveyPkId) else emptyList()
