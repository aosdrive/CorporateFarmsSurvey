...
import pk.gop.pulse.katchiAbadi.data.repository.Resource
...
class SurveyActivity : AppCompatActivity(), SensorEventListener {
    ...
    // Add these properties to the SurveyActivity class
        ...
        lifecycleScope.launch {
            try {
                val syncResult = withContext(Dispatchers.IO) {
                    dropdownRepository.syncDropdownData() // force sync, skip cache check
                }
                Log.d("CascadeDropdown", "Sync result: $syncResult")

                when (syncResult) {
                    is Resource.Success -> Log.d("CascadeDropdown", "✅ Sync success")
                    is Resource.Error -> Log.e("CascadeDropdown", "❌ Sync error: ${syncResult.message}")
                    is Resource.Loading -> TODO()
                }

                loadZones()
            } catch (e: Exception) {
                Log.e("CascadeDropdown", "❌ Exception: ${e.message}", e)
                ToastUtil.showShort(context, "Dropdown error: ${e.message}")
            }
        }
    }
...
// ============================================
    private fun loadZones() {
        lifecycleScope.launch {
            try {
                Log.d("CascadeDropdown", "Loading zones...")

                val zones = withContext(Dispatchers.IO) {
                    dropdownRepository.getZoneEntities()
                }

                allZones.clear()
                allZones.addAll(zones)

                Log.d("CascadeDropdown", "Loaded ${zones.size} zones")

                val zoneNames = zones.map { it.zoneName }
                val zoneAdapter = ArrayAdapter(
                    context,
                    android.R.layout.simple_spinner_item,
                    zoneNames.ifEmpty { listOf("No zones available") }
                )
                zoneAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
                binding.spinnerZone.adapter = zoneAdapter

                // Set up zone selection listener
                binding.spinnerZone.onItemSelectedListener =
                    object : AdapterView.OnItemSelectedListener {
                        override fun onItemSelected(
                            ...
                            if (position >= 0 && position < allZones.size) {
                                selectedZone = allZones[position]
                                Log.d("CascadeDropdown", "Selected Zone: ${selectedZone?.zoneName}")
                                ...
                                // Load divisions for selected zone
                ...
                ToastUtil.showShort(context, "Error loading zones: ${e.message}")
            }
        }
    }

    // ============================================
// LOAD DIVISIONS BY ZONE
// ============================================
    private fun loadDivisions(zoneId: Long) {
        lifecycleScope.launch {
            try {
                Log.d("CascadeDropdown", "Loading divisions for zone ID: $zoneId")

                val divisions = withContext(Dispatchers.IO) {
                    dropdownRepository.getDivisionEntitiesByZone(zoneId)
                }

                allDivisions.clear()
                allDivisions.addAll(divisions)

                Log.d("CascadeDropdown", "Loaded ${divisions.size} divisions")

                val divisionNames = divisions.map { it.divisionName }
                val divisionAdapter = ArrayAdapter(
                    context,
                    android.R.layout.simple_spinner_item,
                    divisionNames.ifEmpty { listOf("No divisions available") }
                )
                divisionAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
                binding.spinnerDivision.adapter = divisionAdapter

                // Set up division selection listener
                binding.spinnerDivision.onItemSelectedListener =
                    object : AdapterView.OnItemSelectedListener {
                        override fun onItemSelected(
                            ...
                            if (position >= 0 && position < allDivisions.size) {
                                selectedDivision = allDivisions[position]
                                Log.d(
                                    "CascadeDropdown",
                                    "Selected Division: ${selectedDivision?.divisionName}"
                                )
                ...
                ToastUtil.showShort(context, "Error loading divisions: ${e.message}")
            }
        }
    }

    // ============================================
// LOAD SECTIONS BY DIVISION
// ============================================
    private fun loadSections(divisionId: Long) {
        lifecycleScope.launch {
            try {
                Log.d("CascadeDropdown", "Loading sections for division ID: $divisionId")

                val sections = withContext(Dispatchers.IO) {
                    dropdownRepository.getSectionEntitiesByDivision(divisionId)
                }

                allSections.clear()
                allSections.addAll(sections)

                Log.d("CascadeDropdown", "Loaded ${sections.size} sections")

                val sectionNames = sections.map { it.sectionName }
                val sectionAdapter = ArrayAdapter(
                    context,
                    android.R.layout.simple_spinner_item,
                    sectionNames.ifEmpty { listOf("No sections available") }
                )
                    ...
                    object : AdapterView.OnItemSelectedListener {
                        override fun onItemSelected(
                            ...
                            if (position >= 0 && position < allSections.size) {
                                ...
                                Log.d(
                                    "CascadeDropdown",
                                    "Selected Section: ${selectedSection?.sectionName}"
                                )
                                ...
                                // Farm blocks for selected section
                ...
                ToastUtil.showShort(context, "Error loading sections: ${e.message}")
            }
        }
    }
    ...
    private fun loadFarms(sectionId: Long) {
        lifecycleScope.launch {
            val farms = withContext(Dispatchers.IO) {
                dropdownRepository.getFarmEntitiesBySection(sectionId)
            }
            allFarms.clear()
    ...
    // ============================================
// LOAD BLOCKS BY SECTION
// ============================================
    private fun loadBlocks(farmId: Long) {
        lifecycleScope.launch {
            try {
                Log.d("CascadeDropdown", "Loading blocks for section ID: $farmId")

                val blocks = withContext(Dispatchers.IO) {
                    dropdownRepository.getBlockEntitiesByFarm(farmId)
                }

                allBlocks.clear()
                allBlocks.addAll(blocks)

                Log.d("CascadeDropdown", "Loaded ${blocks.size} blocks")

                val blockNames = blocks.map { it.blockName }
                val blockAdapter = ArrayAdapter(
                    context,
                    android.R.layout.simple_spinner_item,
                    blockNames.ifEmpty { listOf("No blocks available") }
                )
                blockAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
                binding.spinnerBlock.adapter = blockAdapter

                // Set up block selection listener
                binding.spinnerBlock.onItemSelectedListener =
                    object : AdapterView.OnItemSelectedListener {
                        override fun onItemSelected(
                            ...
                            if (position >= 0 && position < allBlocks.size) {
                                ...
                                Log.d(
                                    "CascadeDropdown",
                                    "Selected Block: ${selectedBlock?.blockName}"
                                )
                            }
                        }
                ...
                ToastUtil.showShort(context, "Error loading blocks: ${e.message}")
            }
        }
    }
    ...
    private fun loadPlots(blockId: Long) {
        lifecycleScope.launch {
            val plots = withContext(Dispatchers.IO) {
                dropdownRepository.getPlotEntitiesByBlock(blockId)
            }
            allPlots.clear()
    ...
    // ============================================
// GET SELECTED VALUES FOR SUBMISSION
// ============================================
    fun getSelectedCascadeValues(): Map<String, String> {
        return mapOf(
    ...
    private fun addSowingPersonEntry() {
        ...
        entryView.findViewById<TextInputEditText>(
            R.id.etSowingCnic
...
//                    Log.d("SurveyActivity", "=== SAVING SURVEY $surveyId ===")
                    ...
                    val realMauzaId = database.activeParcelDao()
                        .getParcelById(parcelId)?.mauzaId
                        ?: sharedPreferences.getLong(
                            Constants.SHARED_PREF_USER_SELECTED_MAUZA_ID,
                            Constants.SHARED_PREF_DEFAULT_INT.toLong()
                        )
                    Log.d("SurveyActivity", "parcelId=$parcelId → realMauzaId=$realMauzaId")

                    // ✅ REPLACE your existing insertSurvey line with this:
                    val surveyId = database.newSurveyNewDao().insertSurvey(
                        survey.copy(mauzaId = realMauzaId)
                    )
                    Log.d("SurveyActivity", "=== SAVING SURVEY $surveyId ===")
